...
class GetProductsListUseCaseTest {
    ...
    @Test
    fun `when repository returns data then usecase returns same ResultBundle`() = runTest {
        // Given
        val expected = ResultBundle(
    ...
    @Test
    fun `when repository returns error then usecase returns same error ResultBundle`() = runTest {
        // Given
        val expected = ResultBundle<List<Product>>(
            data = null,
